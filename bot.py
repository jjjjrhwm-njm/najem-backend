...
def handle_calls(q):
    ...
    if q.data == "u_dashboard": user_dashboard(q.message)
        ...
        # ... باقي شروط الأدمن ...
            ...
            bot.send_message(q.message.chat.id, f"طريقة {m_type}:", reply_markup=mk)
            ...
            bot.register_next_step_handler(msg, process_ban_unban, q.data.replace("choice_manual_", ""))
        elif q.data.startswith("exec_ban_"):
            parts = q.data.split('_')
            mode = f"{parts[2]}_{parts[3]}"
            cid = "_".join(parts[4:])
            update_app_link(cid, {"banned": (mode == "ban_op")})
            bot.send_message(q.message.chat.id, "✅ تم التنفيذ")

# --- [ وظائف الإدارة ] --- 

def list_apps_for_ban(m, mode):
    apps = db_fs.collection("app_links").limit(50).get()
    if not apps: return bot.send_message(m.chat.id, "لا توجد أجهزة.")
    mk = types.InlineKeyboardMarkup(row_width=1)
    for a in apps:
        mk.add(types.InlineKeyboardButton(f"{a.id.split('_')[-1]}", callback_data=f"exec_ban_{mode}_{a.id}"))
...
def show_detailed_users(m):
    try:
        ...
        for user_doc in all_users:
            ...
            msg += f"👤 {udata.get('name', 'غير معروف')} (`{uid}`)\n"
...
def admin_panel(m):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📝 سجلات", callback_data="admin_logs"),
        types.InlineKeyboardButton("🎫 كود", callback_data="gen_key"),
        types.InlineKeyboardButton("🚫 حظر", callback_data="ban_op"),
        types.InlineKeyboardButton("✅ فك حظر", callback_data="unban_op"),
        types.InlineKeyboardButton("📢 إعلان", callback_data="bc_tele")
    )
    bot.send_message(m.chat.id, msg, reply_markup=markup, parse_mode="Markdown") 

# --- [ وظائف النشر الاحترافي المعدلة ] ---
...
def process_upload_desc(m):
    ...
    try:
        ...
        # 5. إرسال الصورة والوصف والزر (الترتيب الذي طلبته)
        bot.send_photo(CHANNEL_ID, photo, caption=decorated_desc, reply_markup=markup, parse_mode="Markdown")
        
        bot.send_message(m.chat.id, "✅ تم النشر بنجاح وسلاسة!")
...
def user_dashboard(m):
    ...
    msg = "👤 **حالة اشتراكاتك:**\n"
    for doc in apps:
        rem = doc.to_dict().get("end_time", 0) - time.time()
...
def redeem_code_step(m):
    ...
    if not vdata: return bot.send_message(m.chat.id, "❌ كود خاطئ.")
    uid = str(m.from_user.id)
    days = vdata.get("days")
    user_data = get_user(uid)
...
def process_trial(m):
    ...
    for doc in apps:
        trial_select_app(m, doc.id)
        break

def trial_select_app(m, cid):
    data = get_app_link(cid)
    if time.time() - data.get("trial_last_time", 0) < 86400:
        return bot.send_message(m.chat.id, "❌ التجربة كل 24 ساعة.")
...
def send_payment(m):
    bot.send_message(m.chat.id, "شراء الاشتراك متاح حالياً عبر الأكواد فقط.")

def wipe_all_data(m):
    bot.send_message(m.chat.id, "تم تصفير البيانات.")

def process_gen_key_start(m):
    if m.text.isdigit(): create_final_key(m, m.text, "all", None)
...
def do_bc_tele(m):
    ...
    for d in users:
        try: bot.send_message(d.id, f"📢 {m.text}")
        except: pass

def do_bc_app(m): pass
