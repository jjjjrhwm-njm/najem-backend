...
# --- إعداداتك الأصلية التي لا تقبل الخطأ ---
...
# --- فحص الحالة (API) - فصل التطبيقات بالـ pkg ---
@app.route('/check')
def check():
    ...
    db = load_db()
    # دمج المعرف مع الحزمة لضمان أن كل تطبيق له اشتراك مستقل
    key = f"{aid}_{pkg}"
...
def start(m):
    ...
    args = m.text.split()
    if len(args) > 1:
        # الربط التلقائي من السمالي (AID_PKG)
        try:
...
def pay_success(m):
    ...
    # تفعيل الاشتراك بناءً على الجهاز والحزمة (payload)
    ...
    save_db(db)
    bot.send_message(m.chat.id, "✅ تم الدفع بنجاح! تم تفعيل اشتراكك.")

@bot.message_handler(func=lambda m: m.text == "🎁 تجربة مجانية (3س)")
def trial(m):
    db = load_db()
    user = db["users"].get(str(m.from_user.id))
    if not user: return bot.send_message(m.chat.id, "❌ ادخل من التطبيق.")
    
    key = f"{user['aid']}_{user['pkg']}"
    if key not in db["app_links"]: db["app_links"][key] = {}
    ...
    if db["app_links"][key].get("trial_used"):
        ...
        db["app_links"][key]["end_time"] = time.time() + 10800 # 3 ساعات
        save_db(db)
        bot.send_message(m.chat.id, "✅ تم تفعيل 3 ساعات! عد للتطبيق.")

# --- لوحة المدير (كما طلبت: تعمل فقط بكلمة نجم1) ---
@bot.message_handler(func=lambda m: m.text == "نجم1")
def admin(m):
    # إزالة شرط الآيدي بناءً على رغبتك ليعمل كما في كودك القديم
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🎫 توليد كود (30 يوم)", callback_data="gen_30"))
    bot.send_message(m.chat.id, "👑 لوحة المدير مفتوحة:", reply_markup=markup)
...
def redeem_final(m):
    ...
    if code in db["vouchers"]:
        ...
        if user:
            key = f"{user['aid']}_{user['pkg']}"
            if key not in db["app_links"]: db["app_links"][key] = {}
            db["app_links"][key]["end_time"] = max(time.time(), db["app_links"][key].get("end_time", 0)) + (days * 86400)
            save_db(db)
