...
# --- [ إضافة: مسار واجهة التطبيق السحابية ] ---
@app.route('/ui')
def server_ui():
    ...
    if len(args) > 1:
        ...
        if "_" in cid:
            ...
            if action == "TRIAL": return trial_select_app(m, cid)
                ...
                msg = bot.send_message(m.chat.id, f"🎫 **أرسل كود التفعيل الآن:**")
...
def show_logs(m):
    ...
    text = "\n".join([d.to_dict().get("text") for d in logs]) if logs else "لا سجلات."
...
def admin_panel(m):
    ...
    msg = (f"👑 **إدارة نجم الإبداع**\n\n"
           f"👥 المستخدمين: `{users_count}` | النشطين: `{active}`")
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
...
def user_dashboard(m):
    ...
    if not apps: return bot.send_message(m.chat.id, "❌ لا تطبيقات مرتبطة.")
    msg = "👤 **حالة اشتراكاتك:**\n"
    for doc in apps:
        data = doc.to_dict()
...
def redeem_code_step(m):
    ...
    if target_type == "user" and target_id != uid: return bot.send_message(m.chat.id, "❌ الكود لمستخدم آخر.")
    user_data = get_user(uid)
    current_cid = user_data.get("current_app")
    def apply_redeem(cid):
        if target_type == "app" and target_id not in cid:
            bot.send_message(m.chat.id, f"❌ لتطبيق محدد: `{target_id}`"); return False
...
def redeem_select_app(m, cid):
    uid = str(m.chat.id); user_data = get_user(uid); vdata = get_voucher(user_data.get("temp_code"))
    if vdata:
        ...
        link = get_app_link(cid); update_app_link(cid, {"end_time": max(time.time(), link.get("end_time", 0)) + (days * 86400)})
        delete_voucher(user_data["temp_code"]); update_user(uid, {"temp_code": firestore.DELETE_FIELD}); bot.send_message(m.chat.id, f"✅ تم التفعيل!")

def process_trial(m):
    uid = str(m.chat.id); apps = db_fs.collection("app_links").where("telegram_id", "==", uid).get()
    if not apps: return bot.send_message(m.chat.id, "❌ لا تطبيق مرتبط.")
    markup = types.InlineKeyboardMarkup(row_width=1)
...
def trial_select_app(m, cid):
    ...
    update_app_link(cid, {"trial_last_time": time.time(), "end_time": new_time}); bot.send_message(m.chat.id, f"✅ تم تفعيل التجربة!")
...
def send_payment(m):
    ...
    bot.send_invoice(m.chat.id, title="VIP", description="تفعيل", invoice_payload=f"pay_{cid}", provider_token="", currency="XTR", prices=[types.LabeledPrice(label="VIP", amount=100)])
...
def process_gen_key_start(m):
    ...
    bot.send_message(m.chat.id, "اختر النوع:", reply_markup=mk)

def process_key_type_selection(q):
    _, _, target, days = q.data.split('_')
    if target == "all": create_final_key(q.message, days, "all", None)
    else:
        mk = types.InlineKeyboardMarkup(); mk.add(types.InlineKeyboardButton("🔍 اختيار", callback_data=f"pick_{target[0]}_list_{days}"), types.InlineKeyboardButton("⌨️ يدوي", callback_data=f"pick_{target[0]}_manual_{days}"))
        bot.send_message(q.message.chat.id, "تحديد الهدف:", reply_markup=mk)

def list_users_for_key(m, days):
    users = db_fs.collection("users").limit(30).get(); mk = types.InlineKeyboardMarkup()
...
def list_apps_for_key(m, days):
    apps = db_fs.collection("app_links").limit(30).get(); mk = types.InlineKeyboardMarkup(); seen = set()
    for a in apps:
        pkg = a.id.split('_')[-1]
...
def expiry_notifier():
    while True:
        try:
            ...
            for doc in links:
                if 82800 < (doc.to_dict().get("end_time", 0) - now) < 86400:
...
def pay_success(m):
    ...
    link = get_app_link(cid); update_app_link(cid, {"end_time": max(time.time(), link.get("end_time", 0)) + (30 * 86400)})
